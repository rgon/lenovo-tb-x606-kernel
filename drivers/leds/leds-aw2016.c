...
static void aw2016_brightness_work(struct work_struct *work)
{
    ...
    printk("AW2016 LEDS brightness %d\n", led->cdev.brightness);
                ...
                const struct i2c_device_id *id)
{
    ...
    if(psy) {
        power_supply_get_property(psy,POWER_SUPPLY_PROP_ONLINE,&reta);
        if(reta.intval == 1) {
            aw2016_write(led_array, AW2016_REG_GCR1, AW2016_CHARGE_DISABLE_MASK | AW2016_CHIP_ENABLE_MASK);
            aw2016_write(led_array, AW2016_REG_GCR2, 0x02);
            aw2016_write(led_array, AW2016_REG_LCFG1,
                    (AW2016_LED_MANUAL_MODE_MASK | 0x03));
            aw2016_write(led_array, AW2016_REG_PWM1, 0xff);
            aw2016_read(led_array, AW2016_REG_LEDEN, &val);
            aw2016_write(led_array, AW2016_REG_LEDEN, val | 1);
        } else
            aw2016_write(led_array, AW2016_REG_GCR1, AW2016_CHARGE_DISABLE_MASK);
    }
#else
    aw2016_write(led_array, AW2016_REG_GCR1, AW2016_CHARGE_DISABLE_MASK);
#endif

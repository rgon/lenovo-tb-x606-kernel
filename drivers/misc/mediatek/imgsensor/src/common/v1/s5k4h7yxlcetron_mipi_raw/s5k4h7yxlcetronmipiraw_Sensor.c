...
#include "s5k4h7yxlcetronmipiraw_Sensor.h"

#define PFX "s5k4h7yxlcetron_camera_sensor"
...
#define DEVICE_VERSION_S5K4H7YXLCETRON    "s5k4h7yxlcetron"
...
extern unsigned char s5k4h7yxlcetron_get_module_id(void);
#endif
//extern unsigned main_sub_flag;
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YXLCETRON_SENSOR_ID,  //S5K4H7YXLCETRON_SENSOR_ID = 0x487E
...
#define INCLUDE_NO_OTP_4H7L 1
#define S5K4H7YXLCETRON_MODULE_ID 0x0a
#define S5K4H7YXLCETRON_OTP_MODULE_DATA_SIZE 20
#define S5K4H7YXLCETRON_FLAG_START_ADDRESS 0x0a04
#define S5K4H7YXLCETRON_CHECKSUM_START_ADDRESS 0x0a05
#define getbit(x,y)   ((x) >> (y)&1)
#define GAIN_DEFAULT       0x0100


typedef struct s5k4h7yxlcetron_otp_data {
...
S5K4H7YXLCETRON_OTP_DATA s5k4h7yxlcetron_otp_data;
...
static int s5k4h7yxlcetron_read_otp_page_data(int page, int start_add, unsigned char *Buff, int size)
{
...
static bool s5k4h7yxlcetron_read_valid_data(int page, int start_add)
{
	unsigned char page_flag[2] = {0};
	s5k4h7yxlcetron_read_otp_page_data(page,start_add,&page_flag[0],1);
	s5k4h7yxlcetron_read_otp_page_data(page,start_add,&page_flag[1],1);
	...
	if (page_flag[0] != 0 || page_flag[1] != 0) {
		...
		s5k4h7yxlcetron_otp_data.page_flag = ((page_flag[0] > page_flag[1])? page_flag[0] : page_flag[1]);
		//LOGE("+++4h7====== s5k4h7yxlcetron_otp_data.page_flag = 0x%x\n",s5k4h7yxlcetron_otp_data.page_flag);
		if (!((getbit(s5k4h7yxlcetron_otp_data.page_flag,6)) && !(getbit(s5k4h7yxlcetron_otp_data.page_flag,7)))) {
			LOG_INF("valid bit 7 = %d,bit 6 = %d\n",getbit(s5k4h7yxlcetron_otp_data.page_flag,7),getbit(s5k4h7yxlcetron_otp_data.page_flag,6));
		...
		s5k4h7yxlcetron_otp_data.page_flag = 0;
		return false;
	}
}

static int s5k4h7yxlcetron_get_vaild_data_page(int start_add)
{
	unsigned short page = 21;
	LOG_INF("read flag .....");
	for (page = 21;page <= 23; page++) {
		if(s5k4h7yxlcetron_read_valid_data(page, start_add)){
			break;
		}else if (23 == page){
			return 0;
		}
...
static int s5k4h7yxlcetron_read_data_kernel(void)
{
	unsigned int page = 0;
	kal_uint16 check = 0;
	unsigned int i = 0;

	page = s5k4h7yxlcetron_get_vaild_data_page(S5K4H7YXLCETRON_FLAG_START_ADDRESS);
	if(!page) return -1;

	s5k4h7yxlcetron_read_otp_page_data(page,S5K4H7YXLCETRON_CHECKSUM_START_ADDRESS,s5k4h7yxlcetron_otp_data.data,S5K4H7YXLCETRON_OTP_MODULE_DATA_SIZE);

	for (i = 0; i < S5K4H7YXLCETRON_OTP_MODULE_DATA_SIZE - 1; i++)
		check += s5k4h7yxlcetron_otp_data.data[i];

	if ((check % 255 + 1) == s5k4h7yxlcetron_otp_data.data[S5K4H7YXLCETRON_OTP_MODULE_DATA_SIZE - 1]) {
		s5k4h7yxlcetron_otp_data.moduleid = s5k4h7yxlcetron_otp_data.data[0];
		rg_ratio = (s5k4h7yxlcetron_otp_data.data[12] << 2 | ((s5k4h7yxlcetron_otp_data.data[14] & 0xc0) >> 6));
		bg_ratio = (s5k4h7yxlcetron_otp_data.data[13] << 2 | ((s5k4h7yxlcetron_otp_data.data[14] & 0x30) >> 4));
		golden_rg_ratio = (s5k4h7yxlcetron_otp_data.data[15] << 2 | ((s5k4h7yxlcetron_otp_data.data[17] & 0xc0) >> 6));
		golden_bg_ratio = (s5k4h7yxlcetron_otp_data.data[16] << 2 | ((s5k4h7yxlcetron_otp_data.data[17] & 0x30) >> 4));
		LOG_ERR("moduleid= 0x%x rg_ratio = %d bg_ratio = %d golden_rg_ratio = %d golden_bg_ratio = %d\n",
			s5k4h7yxlcetron_otp_data.moduleid,rg_ratio,bg_ratio,golden_rg_ratio,golden_bg_ratio);
	}else{
		return -1;
	}
	return 0;
...
static void preview_setting(void)
{
	...
	//write_cmos_sensor_8(0x0100, 0x01);
	}   /*  s5k4h7yxlcetronMIPI_Capture_Setting  */
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001)) + 3;
			...
			// imgsensor_info.module_id = s5k4h7yxlcetron_get_module_id();
...
#if OTP_4H7L
				valid = s5k4h7yxlcetron_read_data_kernel();
#if INCLUDE_NO_OTP_4H7L
				if ((s5k4h7yxlcetron_otp_data.moduleid > 0) && (s5k4h7yxlcetron_otp_data.moduleid < 0xFFFF)) {
#endif
					if (s5k4h7yxlcetron_otp_data.moduleid != S5K4H7YXLCETRON_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						return ERROR_SENSOR_CONNECT_FAIL;
					} else {
						LOG_INF("This is lcetron --->s5k4h7 otp data vaild ...");
						hq_regiser_hw_info(HWID_SUB_CAM, "lcetron s5k4h7yx 8m");
					}
#if INCLUDE_NO_OTP_4H7L
				} else {
...
UINT32 S5K4H7YXLCETRON_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	s5k4h7yxlcetron_MIPI_RAW_SensorInit	*/

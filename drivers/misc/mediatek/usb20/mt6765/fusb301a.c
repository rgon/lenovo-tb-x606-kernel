...
#define FUSB3801_CC_STS_MASK            0x30
/* FUSB STATES */
...
struct fusb301_chip {
	...
	struct wusb3801_wakeup_source    wlock;
	struct mutex mlock;
...
#define fusb_update_state(chip, st) \
	if(chip && st < FUSB_STATE_TRY_SRC) { \
		...
		wake_up_interruptible(&mode_switch_a); \
	}
#define STR(s)    #s
#define STRV(s)   STR(s)
static void fusb301_detach(struct fusb301_chip *chip);
DECLARE_WAIT_QUEUE_HEAD(mode_switch_a);
				...
				char *buf)
{
	...
	if (rc<0)
	{
		pr_err("cannot read FUSB3801_REG_STATUS\n");
		rc = 0xFF;
		ret = snprintf(buf, PAGE_SIZE, "cc_sts (%d)\n", rc);
	}
	rc  =  BITS_GET(rc, FUSB3801_CC_STS_MASK);
	ret = snprintf(buf, PAGE_SIZE, "cc_sts (%d)\n", rc-1);
	return ret;
}
...
static irqreturn_t fusb301_interrupt(int irq, void *data)
{
	...
	__pm_wakeup_event(&chip->wlock.source,
...
static int fusb301_parse_dt(struct fusb301_chip *chip)
{
	...
	data->int_gpio = of_get_named_gpio(dev_node,"fusb301,int-gpio", 0);
...
static int dual_role_set_prop(struct dual_role_phy_instance *dual_role,
	...
	mutex_unlock(&chip->mlock);
	timeout = wait_event_interruptible_timeout(mode_switch_a,
			...
			const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->twork, fusb301_timer_work_handler);
        wakeup_source_init(&chip->wlock.source, "fusb301_wake");
	...
	ret =
	    request_irq(chip->irq_gpio, fusb301_interrupt, IRQF_TRIGGER_FALLING, "fusb301a_int_irq", chip);
...
err3:
	...
	wakeup_source_trash(&chip->wlock.source);

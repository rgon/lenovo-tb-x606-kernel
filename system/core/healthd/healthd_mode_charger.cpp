...
#define UNPLUGGED_SHOSBATTERY_TIME (10 * MSEC_PER_SEC)
#define UNPLUGGED_SUSPEND_TIME (5 * MSEC_PER_SEC)
#define LAST_KMSG_MAX_SZ (32 * 1024)
#define BACKLIGHT_PATH          "/sys/class/leds/lcd-backlight/brightness"
#define CHARGE_LED_PATH        "/sys/class/leds/red/brightness"
#define FULL_LED_PATH        "/sys/class/leds/green/brightness"

#define LOGE(x...) KLOG_ERROR("charger", x);
#define LOGW(x...) KLOG_ERROR("charger", x);
#define LOGV(x...) KLOG_DEBUG("charger", x);
#define LOGI(x...) KLOG_DEBUG("charger", x);
...
struct charger {
    ...
    int64_t next_swi_check;
    int64_t next_poff_check;	
...
static animation::frame batt_anim_frames[] = {
    {
        .name = "charger/charging_10",
        .plugout_name = "charger/plugout_10",

        .disp_time = 500,
        .min_level = 10,
        .max_level = 19,
        .surface = NULL,
        .plugout_surface = NULL,
    },
    {
        .name = "charger/charging_20",
        .plugout_name = "charger/plugout_20",
        .disp_time = 500,
        .min_level = 20,
        .max_level = 29,
        .surface = NULL,
    ...
    {
        .name = "charger/charging_30",
        .plugout_name = "charger/plugout_30",
    ...
    {
        .name = "charger/charging_40",
        .plugout_name = "charger/plugout_40",
    ...
    {
        .name = "charger/charging_50",
        .plugout_name = "charger/plugout_50",
    ...
    {
        .name = "charger/charging_60",
        .plugout_name = "charger/plugout_60",
        .disp_time = 500,
        .min_level = 60,
        .max_level = 69,
        .surface = NULL,
    ...
    {
        .name = "charger/charging_70",
        .plugout_name = "charger/plugout_70",
        .disp_time = 500,
        .min_level = 70,
        .max_level = 79,
        .surface = NULL,
    ...
    {
        .name = "charger/charging_80",
        .plugout_name = "charger/plugout_80",
        .disp_time = 500,
        .min_level = 80,
        .surface = NULL,
        .max_level = 89,
    ...
    {
        .name = "charger/charging_90",
        .plugout_name = "charger/plugout_90",
        .disp_time = 500,
        .min_level = 90,
        .max_level = 99,
        .surface = NULL,
    ...
    {
	.name = "charger/charging_100",
	.plugout_name = "charger/plugout_100",
	.disp_time = 500,
	.min_level = 100,
	.max_level = 100,
	.surface = NULL,
...
void stop_backlight(void);
int lights_chgon(void);
int lights_chgfull(void);
...
static void set_chargelight_charging(void)
{
	lights_chgon();
#if 0
    int fdred=-1, fdgreen=-1;
    LOGE("set_chargelight_charging\n");

    fdred = open(CHARGE_LED_PATH, O_RDWR);
    if (fdred < 0) {
        LOGE("Could not open charge light node : %s\n", strerror(errno));
        goto cleanup;
    }
    fdgreen = open(FULL_LED_PATH, O_RDWR);
    if (fdgreen < 0) {
        LOGE("Could not open full light node : %s\n", strerror(errno));
        goto cleanup;
    }
	
    if (write(fdred, "255",3) < 0) {
        LOGE("Could not write to charge light node : %s\n", strerror(errno));
        goto cleanup;
    }
    if (write(fdgreen, "0",1) < 0) {
        LOGE("Could not write to full light node : %s\n", strerror(errno));
        goto cleanup;
    }	
cleanup:
    if (fdred >= 0)
...
static void set_chargelight_full(void)
{
	lights_chgfull();
#if 0
    int fdred=-1, fdgreen=-1;
    LOGE("set_chargelight_full\n");
    fdred = open(CHARGE_LED_PATH, O_RDWR);
    if (fdred < 0) {
        LOGE("Could not open charge light node : %s\n", strerror(errno));
        goto cleanup;
    ...
    if (write(fdred, "0",1) < 0) {
        LOGE("Could not write to charge light node : %s\n", strerror(errno));
        goto cleanup;
    }
    if (write(fdgreen, "255",3) < 0) {
        LOGE("Could not write to full light node : %s\n", strerror(errno));
        goto cleanup;
    }	
cleanup:
...
static void update_screen_state(charger* charger, int64_t now) {
   ...
   if(charger->next_swi_check == -1)
        charger->next_swi_check = now + UNPLUGGED_SHOSBATTERY_TIME;
    /* animation is over, blank screen and leave */
  //  if (batt_anim->num_cycles > 0 && batt_anim->cur_cycle == batt_anim->num_cycles) {
   if(now > charger->next_swi_check ){
        reset_animation(batt_anim);
        charger->next_screen_transition = -1;
        charger->next_swi_check = -1;
        set_backlight(false);
        healthd_draw->blank_screen(true);
    ...
    healthd_draw->redraw_screen(charger->batt_anim, charger->charger_connected);
...
static void handle_power_supply_state(charger* charger, int64_t now) {
    ...
    if (!charger->charger_connected) {
	   ...
	   charger->next_poff_check = now + UNPLUGGED_SUSPEND_TIME;
            LOGW("[%" PRId64 "] device unplugged: shutting down in %" PRId64 " (@ %" PRId64 ")\n",
                 now, (int64_t)UNPLUGGED_SHUTDOWN_TIME, charger->next_pwr_check);
        } else if (now >= charger->next_pwr_check) {
            LOGW("[%" PRId64 "] shutting down\n", now);
            reboot(RB_POWER_OFF);
        } else if((now > charger->next_poff_check) && (now < charger->next_pwr_check)){
          		  //if(charger->batt_anim->run)
       		     gr_fb_blank(true);
         	     reset_animation(charger->batt_anim);
         } else if(now <= charger->next_poff_check){
                           kick_animation(charger->batt_anim);
         }else{
...
void healthd_mode_charger_battery_update(android::BatteryProperties* props) {
    ...
    batt_prop = props;
    if(100 == batt_prop->batteryLevel && 1!=chargelight_flag){
        set_chargelight_full();
        chargelight_flag = 1;
    }
    if(100 > batt_prop->batteryLevel && 2!=chargelight_flag){
        set_chargelight_charging();
        chargelight_flag = 2;
...
animation* init_animation() {
    ...
    if (!parse_success) {
        ...
        battery_animation.frames = batt_anim_frames;
        battery_animation.num_frames = ARRAY_SIZE(batt_anim_frames);
    }
...
void healthd_mode_charger_init(struct healthd_config* config) {
    ...
    ret = res_create_display_surface(anim->fail_file.c_str(), &anim->surf_unknown);
    if (ret < 0) {
        LOGE("Cannot load custom battery_fail image. Reverting to built in: %d\n", ret);
        ret = res_create_display_surface("charger/battery_fail", &anim->surf_unknown);
        if (ret < 0) {
            LOGE("Cannot load built in battery_fail image\n");
            anim->surf_unknown = NULL;
        }
    }
    ret = res_create_display_surface("charger/plugout_100", &anim->surf_full);
    if (ret < 0) {
        LOGE("Cannot load battery_fail image\n");
        anim->surf_full = NULL;
    }
    ret = res_create_display_surface("charger/charging_text_percent", &anim->surf_percent);
    if (ret < 0) {
        LOGE("Cannot load percent image\n");
       anim->surf_percent = NULL;
    }
...
for(i = 0; i < charger->batt_anim->num_frames; i++)
    {
        ret = res_create_display_surface(charger->batt_anim->frames[i].name, &(charger->batt_anim->frames[i].surface));
        if (ret < 0) {
            LOGE("Cannot load image %s\n",charger->batt_anim->frames[i].name);
            charger->batt_anim->num_frames = 0;
            charger->batt_anim->num_cycles = 1;
            charger->batt_anim->frames[i].surface = NULL;
        }

	ret = res_create_display_surface(charger->batt_anim->frames[i].plugout_name, &(charger->batt_anim->frames[i].plugout_surface));
        if (ret < 0) {
            LOGE("Cannot load low image %s\n",charger->batt_anim->frames[i].plugout_name);
            charger->batt_anim->num_frames = 0;
            charger->batt_anim->num_cycles = 1;
           charger->batt_anim->frames[i].plugout_surface = NULL;
        }
    }

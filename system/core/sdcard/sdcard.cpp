...
/* modify for limit write to data partition, begin
 * @orig
                           mode_t mask, bool derive_gid, bool default_normal, bool use_esdfs) {
 */
                           mode_t mask, bool derive_gid, bool default_normal, bool use_esdfs,int reserved_mb,int reserved_uid) {
...
*/
        auto opts = android::base::StringPrintf("fsuid=%d,fsgid=%d,%smask=%d,userid=%d,gid=%d,reserved_mb=%d,reserved_uid=%d",
                                                fsuid, fsgid, new_opts.c_str(), mask, userid, gid,reserved_mb,reserved_uid);
/* modify for limit write to data partition, end */      
...
/* modify for limit write to data partition, begin
         ...
         */
                              derive_gid, default_normal, use_esdfs,0,0);
    } else {
 ...
 * @orig
                         bool derive_gid, bool default_normal, bool use_esdfs) {
 */
                         bool derive_gid, bool default_normal, bool use_esdfs,int reserved_mb,int reserved_uid) {
/* modify for limit write to data partition, end */
 ...
 /* modify for limit write to data partition, begin
             ...
             */
                            AID_SDCARD_RW, 0006, derive_gid, default_normal, use_esdfs,reserved_mb,reserved_uid) ||
 /* modify for limit write to data partition, end */
...
static int usage() {
    LOG(ERROR) << "usage: sdcard [OPTIONS] <source_path> <label>"
	       ...
	       << "    -r: specify reserved space (M)"
               << "    -R: specify uid, which, greater than it can not use reserved space";
    return 1;
}
...
/* modify for limit write to data partition, begin
     ...
     */
    while ((opt = getopt(argc, argv, "u:g:U:mwGir:R:")) != -1) {
/* modify for limit write to data partition, end */
        switch (opt) {
            case 'u':
 ...
 /* add for limit write to data partition, begin */
            case 'r':
                reserved_mb = strtoul(optarg, NULL, 10);
                break;
            case 'R':
                reserved_uid = strtoul(optarg, NULL, 10);
                break;
            /* add for limit write to data partition, end */
            case '?':
            default:
...
/* modify for limit write to data partition, begin
     ...
     */
    run_sdcardfs(source_path, label, uid, gid, userid, multi_user, full_write, derive_gid,
                 default_normal, !should_use_sdcardfs(), reserved_mb, reserved_uid);
/* modify for limit write to data partition, end */
    return 1;
}

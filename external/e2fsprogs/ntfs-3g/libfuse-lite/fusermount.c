                    ...
                    char **mnt_optsp)
{
	...
	fprintf(stderr, "%s: do_mount opts=%s \n", progname,opts);
    for (s = opts, d = optbuf; *s;) {
        ...
        if (begins_with(s, fsname_str)) {
			...
			{
					strlen1 = strlen(sep);
					fprintf(stderr, "sep-1 = %s\n",sep);
					sep++;
					sep = strchr(sep, ',');
					if(sep)
					{
						strlen2 = strlen(sep);
						fprintf(stderr, "sep-2 = %s\n",sep);
						strlen_add = strlen1 - strlen2;
						len = len + strlen_add;
						fprintf(stderr, "strlen1 = %d strlen2 = %d strlen_add = %d\n",strlen1,strlen2,strlen_add);
					}
			} else {
					fprintf(stderr, "sep-0 \n");
			}
    ...
    *d = '\0';
	...
	fprintf(stderr, "%s: do_mount  fsname=%s  mnt=%s type=%s flags=%d optbuf=%s\n", progname,fsname,mnt,type,flags,optbuf);
	
    res = mount(source, mnt, type, flags, optbuf);

	fprintf(stderr, "%s: do_mount res=%d errno=%d strerror=%s\n", progname,res,errno,strerror(errno));

	      ...
	      IN struct iw_request_info *prIwReqInfo, IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	case PRIV_CMD_SET_TX_POWER:
	{
		...
		if (u4CmdLen > 4)
			return -EINVAL;
		if (copy_from_user(setting, prIwReqData->data.pointer, u4CmdLen*sizeof(UINT_32)))
			return -EFAULT;
		...
		if (setting[0] == 0 && prIwReqData->data.length == 4 /* argc num */) {
			...
			rStatus = kalIoctl(prGlueInfo,
					   wlanoidSetTxPower,
					   prTxpwr,
					   sizeof(SET_TXPWR_CTRL_T), FALSE, FALSE, TRUE, FALSE, &u4BufLen);
		} else
			return -EFAULT;
#else
		...
		if (setting[0] == 0 && prIwReqData->data.length == 4 /* argc num */) {
			...
			rStatus = kalIoctl(prGlueInfo,
				wlanoidSetTxPower,
				prTxpwr,
				sizeof(SET_TXPWR_CTRL_T), TRUE, FALSE, TRUE, FALSE, &u4BufLen);
